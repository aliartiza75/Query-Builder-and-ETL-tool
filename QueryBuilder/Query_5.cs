...
{
    public partial class Query_5 : Form
    {
        ...
        private void load_district_tehsil_Click(object sender, EventArgs e)
        {
            district_tehsil.Items.Clear();
        ...
        private void execute_query_Click(object sender, EventArgs e)
        {
            string NewconnectionString = "Data Source=WNDOWS-SAVJEG8; Initial Catalog= " + source_databases.SelectedItem.ToString() + "; Integrated Security=True";
            SqlConnection openCon = new SqlConnection(NewconnectionString);
            string sqlselectQuery = " SELECT * from Variety_of_Corps"
                        + " left Join Fact_Table on Variety_of_Corps.Variety_of_Corps_ID = Fact_Table.Variety_of_Corps_ID  where  ( Fact_Table.Mouza_Town_ID = (Select Mouza_Town_ID from Mouza_Town where Mouza_Town_Name = '" + mouza_town.SelectedItem.ToString() + "' ))";
            ...
            using (openCon)
            {
                using (SqlDataReader sdr = sqlCmd.ExecuteReader())
                {
                    ...
                    while (sdr.Read()) // Start Reading records 
                    {
                        ss = sdr[0].ToString().Trim() + " " + sdr[1].ToString().Trim() + " " + sdr[2].ToString().Trim();
                        if (pestiside_used.ContainsKey(ss))
                        {
                            pestiside_used[ss] = pestiside_used[ss] + 1;
                        }
                        else
                        {
                            pestiside_used.Add(ss, 0);
                        }
        ...
        private void Query_5_Load(object sender, EventArgs e)
        {
